...
**인식되는 행정구역:**
- 도/특별시/광역시/특별자치시/특별자치도
- 시/군
- 구

**형식 규칙:**
1. **특별시/광역시** → `{시} {구}` (예: 서울특별시 강남구)
2. **도** → `{도} {시/군}` (예: 경기도 성남시, 강원도 홍천군)
...
**형식 규칙:**
1. **東京都/北海道** → `{都/道} {区}` (예: 東京都 渋谷区)
2. **府/県** → `{府/県} {市}` (예: 大阪府 大阪市, 神奈川県 横浜市)

### 기타 국가

- `{city}` 단위로 저장 (예: US:San Francisco)
